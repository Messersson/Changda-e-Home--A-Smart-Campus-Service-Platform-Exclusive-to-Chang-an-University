    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>论坛管理</span>
          <el-select v-model="filterStatus" placeholder="筛选状态" style="width: 120px" @change="loadPosts">
      ...
      <el-table :data="posts" stripe>
        ...
        <el-table-column label="图片" width="80">
          <template #default="{ row }">
            {{ row.images.length > 0 ? '有' : '无' }}
          </template>
        </el-table-column>
        <el-table-column prop="likes" label="点赞" width="80" sortable />
        <el-table-column prop="comments.length" label="评论" width="80" sortable />
...
const loadPosts = async () => {
  try {
    const res = await adminApi.getForumPosts({ status: filterStatus.value })
...
const updateStatus = async (post, status) => {
  try {
    await adminApi.updateForumPostStatus(post.id, { status })
    post.status = status
